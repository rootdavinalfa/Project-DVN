...
void LoadingMenu::Draw(SDL_Renderer* rR) {
	if(loadingType) {
		...
		CCFG::getText()->DrawCenterX(rR, "TOMBOL LARI DENGAN " + CCFG::getKeyString(CCFG::keyIDShift), 390, 16);
		CCFG::getText()->DrawCenterX(rR, "NOTE: GAME INI MERUPAKAN PENGEMBANGAN DARI JAKOWSKI OLEH ROOTDAVINALFA DIBAWAH LISENSI GNU  ", 400, 16);
	} else {
		CCore::getMap()->DrawGameLayout(rR);
